...
|---|---|
| Native Accumulo conditional writer | The first M3 slice exposes a public exact-row API over Accumulo's native conditional-update RPC with bounded pre-submission routing retries. M3 still must build and fault-test the claims, fencing, publication, and reconciliation state machines on top of it. |
