...
principal, tenant, source, or Accumulo authorization labels. The gRPC server
passes `context.Context` through but does not authenticate it. Therefore
authorization is **not current implemented Explorer behavior**.

Proposed northbound deployments authenticate with a trusted interceptor and
inject an immutable decision containing:
...
Embedded callers use an equally trusted host resolver. Untrusted
...
The adapter uses a stricter grammar than the low-level Accumulo API, which
...
- Implement the shared canonical codec, scorer/analyzer versions, auth
  resolver, and conformance harness.
