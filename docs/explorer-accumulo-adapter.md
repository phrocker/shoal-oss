...
| Accumulo writes | `accumulo/mutation.go` provides one-row puts/deletes. `batch_writer.go` provides bounded buffering, durability, safe retries, and explicit ambiguous-partial-commit failure. `conditional_writer.go` exposes exact-row absence/value conditions, optional exact timestamps, and Accepted/Rejected/Unknown results over the native Accumulo conditional-update protocol. `pkg/explorer/coordination` provides bounded deterministic M3 record/key vocabulary for TXNs/manifests, guards and write intents, reservations/outcomes/checkpoints, policy copies, BUILDING/SEALED index-generation manifests with append-only activations, leases/retirement/history floors, and writer-authority/backend observations. | BatchWriter is not atomic across mutations. The codec and transition helpers do not implement allocator mutations, publication, durable authority storage, backend routing, recovery/reconciliation workers, or authoritative conditional readback state machines. Full M3 is not complete. |
...
|---|---|
| Native Accumulo conditional writer | The first M3 slices expose a public exact-row API over Accumulo's native conditional-update RPC and the bounded coordination record/key vocabulary in `pkg/explorer/coordination`, including guards, policy-copy metadata, BUILDING/SEALED index manifests and append-only activation records, leases/history/retirement, and authority observations. Allocator/publication mutations, LPART commit copies, durable catalogs/mirrors, routing, and recovery/reconciliation workers remain unimplemented; these slices do not make full M3 complete. |
