...
EXPECTED_METADATA_FIELDS = {
    ...
    "Shoal reference": (
        "`phrocker/shoal-oss` revision 50 is pinned to latest-main merge "
        "`fe684bf` immediately before matrix serialization"
    ),
...
EXPECTED_DOCUMENT_STATUS_SNIPPETS = (
    ...
    "Revision 50 \u2014 closes the exact 28 configuration, credential, connector",
...
EXPECTED_SCOPE_MANIFEST_HEADER = (
    ...
    "# Shoal source: phrocker/shoal-oss@fe684bf.",
...
def validate_status_narratives(
    ...
    expected_phrases = [
        ...
        (COUNTS_SECTION_HEADING, f"**Exactly {status_counts['Covered']} rows are `Covered`: the 231 rows delivered through revision 49 plus the exact 28 configuration, credential, connector/session, and dynamic-attribute rows completed in revision 50. Revision 50 adds sixteen `SB-CFG` rows, all eleven issue-owned `SB-CONN` rows, and `SB-STAT-038`, each with named Go/C/Python evidence.**"),
