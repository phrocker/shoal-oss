...
EXPECTED_METADATA_FIELDS = {
    ...
    "Shoal reference": (
        "`phrocker/shoal-oss` revision 51 is pinned to latest-main merge "
        "`e68ea17` immediately before matrix serialization"
    ),
...
EXPECTED_DOCUMENT_STATUS_SNIPPETS = (
    ...
    "Revision 51 \u2014 closes the exact 28 configuration, credential, connector",
...
EXPECTED_APPROVED_DIVERGENCE_ROW_COUNT = 92
...
EXPECTED_SCOPE_MANIFEST_HEADER = (
    ...
    "# Shoal source: phrocker/shoal-oss@e68ea17.",
...
def validate_status_narratives(
    ...
    expected_phrases = [
        ...
        (COUNTS_SECTION_HEADING, f"**Exactly {status_counts['Covered']} rows are `Covered`: the 291 rows delivered through revision 50 plus the exact 28 configuration, credential, connector/session, and dynamic-attribute rows completed in revision 51. Revision 51 adds sixteen `SB-CFG` rows, all eleven issue-owned `SB-CONN` rows, and `SB-STAT-038`, each with named Go/C/Python evidence.**"),
