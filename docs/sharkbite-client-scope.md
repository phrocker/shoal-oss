...
Status: Accepted, normative for `docs/sharkbite-compatibility.md` revision 47.
...
`docs/sharkbite-compatibility-revision47-scope.tsv`:
...
4. **Optional** — PyTorch, pandas/pandashark, the dead embedded extension, or
   historical C++ header/test surface. These may be delivered separately but
   do not block the core client package.
...
No other prefix-level bulk exclusion is permitted.
...
missing implementation into `Covered`. Core rows with `Missing Go`,
`Missing C ABI`, or `Behavior mismatch` remain `Required gap`.
...
Revision 47 therefore keeps `SB-XCUT-014`, `SB-XCUT-019`, and `SB-PKG-008` as
required runtime/environment gaps.
