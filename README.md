...
| Local→Accumulo promotion (`internal/promotion`, Bulk Import V2) | partial — load-mapping/staging + FATE `TABLE_BULK_IMPORT2` submission through the manager, verified against upstream Java source; destination must already be split to match, and an exact-boundary-value gap plus live-cluster verification, cutover protocol, and ongoing fan-in remain open ([design](docs/promotion.md)) |
