...
| Accumulo tablet-server lifecycle + ServiceLock | read-only process wired — `shoal-tserver` owns fenced hosting, manager RPCs, hosted-only scans, and ZooKeeper registration; ingest remains unsupported pending TabletIngest/minor-compaction integration with the landed WAL authority ([design](docs/tserver-hosting-lifecycle.md)) |
...
| Manager-facing tablet lifecycle RPC adapter (`internal/tserverrpc`) | wired — fenced assignment/unload/status/control, metadata-backed loads, cancellation, failover-aware reporting, hosted-only stateful scans, and exact service advertisement are available in `cmd/shoal-tserver`; `TABLET_INGEST` is deliberately absent ([lifecycle](docs/tserver-hosting-lifecycle.md)) |
