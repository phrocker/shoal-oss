...
| Accumulo tablet-server lifecycle + ServiceLock | read-only process wired; the TabletIngest session/WAL/memtable/minc composition is implemented behind a fail-closed production metadata-authority boundary, so `shoal-tserver` does not yet advertise ingest ([lifecycle](docs/tserver-hosting-lifecycle.md) · [ingest](docs/tablet-ingest-service.md)) |
...
| Manager-facing tablet lifecycle RPC adapter (`internal/tserverrpc`) | wired — assignment attempts now flow to writable backends for exact data-plane fencing; `TABLET_INGEST` remains deliberately absent until the production conditional metadata authority initializes ([lifecycle](docs/tserver-hosting-lifecycle.md)) |
