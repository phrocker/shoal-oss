...
func (b *Backend) Create(_ context.Context, path string) (storage.Writer, error) {
	...
	target, err := resolveCreateTarget(path)
	if err != nil {
		return nil, err
	}
...
func resolveCreateTarget(path string) (string, error) {
	info, err := os.Lstat(path)
	if errors.Is(err, fs.ErrNotExist) {
		return path, nil
	}
	if err != nil {
		return "", fmt.Errorf("local: inspect existing file %s: %w", path, err)
	}
	if info.Mode()&os.ModeSymlink != 0 {
		return "", fmt.Errorf("local: symlink destination %s is not supported for atomic replacement", path)
	}
	return path, nil
}
