...
func ComposeHosted(ctx context.Context, config HostConfig) (*HostedServices, error) {
	...
	lifecycleRecorder, err := explorerfleet.NewLifecycleRecorder(
		hostedInteractionSink{
			storage: config.InteractionStorage,
			result:  config.Interaction,
		})
	if err != nil {
	...
	registry, err := explorerfleet.Compose(
		config.Runtime, config.Resolver, lifecycleRecorder, config.Snapshots,
		config.Executors,
	...
	if !ok || trustedInteractions == nil {
		return nil, shoal.NewError(
			shoal.ErrorInvalidArgument,
			"fleet event interaction storage result sink is required")
	}
	events, actionEvents, err := ComposeWithPublisher(
		config.Runtime, config.Domain, config.Resolver, config.Generations,
		recorder, trustedInteractions, registry, cursorKey, config.Clock)
	if err != nil {
		return nil, err
	}
...
// Mount adds both products to the authenticated Handler.
func (s *HostedServices) Mount(handler *webapi.Handler) error {
	if s == nil || handler == nil || s.registry == nil || s.events == nil {
		return shoal.NewError(shoal.ErrorInvalidArgument, "hosted fleet services are required")
	}
	if err := handler.MountFleetRegistry(s.registry); err != nil {
		return err
	}
