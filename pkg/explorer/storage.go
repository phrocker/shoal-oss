...
func validateSnapshotDelta(record persistedSnapshot) error {
	...
	if err := validateSnapshotIDDelta(
		"node", record.AddedNodeIDs, record.RemovedNodeIDs,
	); err != nil {
		return err
	}
	return validateSnapshotIDDelta(
		"edge", record.AddedEdgeIDs, record.RemovedEdgeIDs)
}

func validateSnapshotIDDelta(
	kind string, added, removed []shoal.ID,
) error {
	for _, ids := range [][]shoal.ID{added, removed} {
		for index, id := range ids {
			...
			if index > 0 && shoal.CompareID(ids[index-1], id) >= 0 {
				return fmt.Errorf("snapshot %s IDs are not canonical", kind)
			}
		}
	}
	for _, id := range added {
		index := sort.Search(len(removed), func(index int) bool {
			return shoal.CompareID(removed[index], id) >= 0
		})
		if index < len(removed) && removed[index] == id {
			return fmt.Errorf(
				"snapshot %s cannot be both added and removed", kind)
		}
	}
	return nil
}
...
func persistedSnapshotsEqual(left, right persistedSnapshot) bool {
	return left.ID == right.ID &&
		...
		reflect.DeepEqual(left.AddedEdgeIDs, right.AddedEdgeIDs) &&
