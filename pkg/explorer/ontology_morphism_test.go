...
func TestInterpretAssertionsIgnoresDisconnectedPublishedPredecessor(
	t *testing.T,
) {
	corpus, err := Open(filepath.Join(t.TempDir(), "corpus"))
	if err != nil {
	...
	defer corpus.Close()
	schema, _ := ontology.NewOntologySchema("branch", "Branch", "", nil)
	property, _ := ontology.NewPropertyDefinition(
		"name", "Name", "", ontology.ValueString, nil, nil)
	person, _ := ontology.NewConceptDefinition(
		"person", "Person", "", []shoal.ID{property.ID()}, nil)
	at := time.Date(2026, 9, 6, 12, 0, 0, 0, time.UTC)
	version := func(name string, offset time.Duration) ontology.OntologyVersion {
		value, versionErr := ontology.NewOntologyVersion(
			schema, name, at.Add(offset),
	...
	base := version("1", 0)
	disconnected := version("2", time.Second)
	target := version("3", 2*time.Second)
	publish := func(base ontology.OntologyVersion, author string) {
		t.Helper()
		proposal, proposalErr := ontology.NewGovernedProposal(
			schema, base, target, author, "publish target",
			at.Add(3*time.Second), nil)
		if proposalErr != nil {
			t.Fatal(proposalErr)
		}
		ctx := context.Background()
		if proposalErr = corpus.CreateOntologyProposal(
			ctx, proposal, base,
		); proposalErr != nil {
			t.Fatal(proposalErr)
		}
		for index, state := range []ontology.ProposalState{
			...
			proposal, proposalErr = corpus.TransitionOntologyProposal(
				ctx, proposal.ID(), state, "governor", "approved",
				at.Add(time.Duration(index+4)*time.Second))
			if proposalErr != nil {
				t.Fatal(proposalErr)
			}
		}
	}
	publish(base, "primary")
	publish(disconnected, "disconnected")
	...
	targetIdentity, _ := ontology.NewOntologyIdentity(target)
	read, err := corpus.InterpretAssertions(
		context.Background(), []ontology.Assertion{assertion}, targetIdentity)
	if err != nil || len(read) != 1 || !read[0].Resolved() {
		t.Fatalf("disconnected predecessor affected selected path: %#v, %v",
			read, err)
	}
}
